...
extension ChatMessage: SocketData {
    func socketRepresentation() throws -> SocketData {
        // add sender name and message preview to server for notification usage
        var messageData = try self.asDictionary()
        messageData["senderName"] = UserManager.shared.getChatUser()?.name ?? ""
        messageData["messagePreview"] = self.toMessagePreview().preview
        return messageData
    }
}
